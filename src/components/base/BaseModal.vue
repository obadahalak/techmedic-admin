...
const emit=defineEmits(['chnageStatus']);
...
function close() {
  status.value = false;
  emit('chnageStatus',status.value);  
}
...
<template>
  <div id="modal" v-bind="$attrs" :class="{ 'hidden': !status }" class=" fixed z-10 inset-0 overflow-y-auto">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900">{{ type }} {{ title }}</h3>


              <div class="mt-4  flex justify-center">

                <li class="text-sm  text-red-600" v-for="error in  company.error"> {{ error }}</li>
